...
const FeaturedProducts = () => {
  ...
  const InfoSection = () => {
    return (
      <section className="grid bg-[#EEEEEE]  grid-cols-2 sm:grid-cols-4 gap-10 p-20 pt-0">
        {info.map((i, index) => (
...
function Special() {
  return (
    <>
      <FeaturedProducts/>
      <Slider heading={"Trending Today"} slide={3} arr={trending}/>
      <LimitedOffer /> 
      <Slider heading={"Categories"} slide={4}  arr={categories}/>
      <InfoSection />
    </>
  )
}
